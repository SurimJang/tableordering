<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            결제내역 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            결제내역
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            <Number label="PaymentAmount" v-model="value.paymentAmount" :editMode="editMode" :inputUI="''"/>
